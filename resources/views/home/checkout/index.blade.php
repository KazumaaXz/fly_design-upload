                ...
                <div class="card shadow-sm border-0">
                    <div class="card-body p-4">
                        ...
                        <ul class="list-group list-group-flush mb-3">
                            @foreach ($cartItems as $item)
                                @if ($item->product)
                                    @php
                                        ...
                                        $itemDiscount = $item->discount_at_add; // misal: 20 untuk 20%

                                        $discountAmountPerUnit = 0;
                                        $finalItemPrice = $itemPrice;

                                        if ($itemDiscount > 0) {
                                            $discountRate = $itemDiscount / 100;
                                            $discountAmountPerUnit = $itemPrice * $discountRate;
                                            $finalItemPrice = $itemPrice - $discountAmountPerUnit;
                                        }

                                        $lineSubtotal = $itemPrice * $itemQuantity;
                                        $lineDiscount = $discountAmountPerUnit * $itemQuantity;
                                        $lineTotal = $finalItemPrice * $itemQuantity;
                                    ...
                                    <li
                                        ...
                                        <div>
                                            ...
                                            <small class="text-muted">
                                                {{ $itemQuantity }} x Rp{{ number_format($itemPrice, 0, ',', '.') }}
                                                @if ($itemDiscount > 0)
                                                    (diskon {{ $itemDiscount }}% =
                                                    -Rp{{ number_format($discountAmountPerUnit, 0, ',', '.') }} / item)
                                                @endif
                                            </small>
                                        </div>
                            ...
                            <li
                                class="list-group-item d-flex justify-content-between align-items-center px-0 py-2 text-danger">
                                Diskon:
                            ...
                            <li
                                ...
                                Total Pembayaran:
                                <span
                                    class="fs-4 text-success">Rp{{ number_format($finalTotalAmount, 0, ',', '.') }}</span>
                            </li>
                        </ul>
                        <p class="text-muted small text-center mt-3">
                            Dengan menekan "Bayar Sekarang", Anda menyetujui Syarat dan Ketentuan kami.
                        </p>
                    </div>
                </div>
            </div>
